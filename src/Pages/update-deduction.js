...
function UpdateDeduction(){
    ...
    const initialInputState = { deductionId:parseInt(0), deductionName: "", deductionType: "" };
    ...
    useEffect(() => {  
        const GetData = async () => {  
          const result = await axios(process.env.REACT_APP_API+"DeductionAPI/"+params.id);  
    ...
    const handleFinalSubmit = e => {
        e.preventDefault();
        // debugger;
        const data = {deductionId:parseInt(params.id), deductionName:eachEntry.deductionName, deductionType:eachEntry2.deductionType}
        axios.put(process.env.REACT_APP_API+"DeductionAPI/"+params.id,data).then(navigate("/ViewDeductions"));
